...
from statsmodels.genmod.tests.results import res_R_var_weight
...
def zap_spider(zap, target):
    ...
    spider_scan_id = zap.spider.scan(target, subtreeonly="true")
    time.sleep(5)
    ...
    print "FINAL RESULT SCANNING \n"
    print zap.spider.full_results(spider_scan_id)
    print "\n PRINTING FULL RESULTS COMPLETED"
...
def zap_ajax_spider(zap, target, max_time):
    ...
    zap.ajaxSpider.scan(target, inscope="https://terra.arcesium.com/uds", subtreeonly="true", contextname="uds")
    time.sleep(5)
...
def zap_print_context(zap):
    res = zap.context.context_list
    print "\n PRINTING CONTEXTS: "
    print res

    print "\n EXCLUDED REGEX: "
    print zap.context.exclude_regexs('https://terra.arcesium.com')

    print "\n INCLUDED REGEX: "
    print zap.context.include_regexs('https://terra.arcesium.com')
